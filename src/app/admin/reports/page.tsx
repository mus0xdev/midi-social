...
export default function AdminReportsPage() {
  ...
  async function updateStatus(id: string, status: ReportRow["status"]) {
    ...
    if (!error) setReports((items) => status === "dismissed" ? items.filter((report) => report.id !== id) : items.map((report) => report.id === id ? { ...report, status } : report));
  }

  async function deleteReportedMidi(report: ReportRow) {
    if (!report.midi_id || !window.confirm("Delete this MIDI permanently?")) return;
    const { error } = await supabase.rpc("admin_delete_midi", { target_midi_id: report.midi_id });
    if (!error) setReports((items) => items.filter((item) => item.id !== report.id));
  }
